...
export default function DailyReflection() {
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase.from('daily_reflections').insert({
      ...
      student_name: student?.name || 'אנונימי',
      ...
      <PageTransition>
        <div className="screen-container">
          <div className="text-center py-16">
            ...
            <p className="text-base text-muted-foreground mb-6">הדיווח נשמר בהצלחה.</p>
    ...
    <PageTransition>
      <div className="screen-container">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6">
          ...
          <p className="text-base text-muted-foreground mt-1">דרג/י את עצמך מ-1 עד 5</p>
        </motion.div>
            ...
            >
                ...
                {[1, 2, 3, 4, 5].map(n => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 rounded-xl py-2.5 text-center transition-all border-2 ${
                        selected
                          ? 'bg-primary border-primary text-primary-foreground scale-105 shadow-md'
                          : 'bg-secondary/40 border-transparent text-foreground hover:bg-secondary/70'
                      }`}
                    >
              ...
              {ratings[cat.key] && (
                <motion.p
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-xs text-center text-primary mt-2 font-medium"
                >
                  {ratingLabels[ratings[cat.key]]}
                </motion.p>
              )}
            </motion.div>
          ))}
