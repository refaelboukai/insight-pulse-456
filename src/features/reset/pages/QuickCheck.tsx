...
import { emotionalStates } from '@reset/data/skills';
...
import { playSelect } from '@reset/hooks/useSoundEffects';

const iconMap: Record<string, any> = {
  Smile, Flame, AlertTriangle, CloudRain, Waves, Users, BookOpen, Battery, HelpCircle, AlertOctagon,
};
...
export default function QuickCheck() {
  ...
  const handleSelect = (stateId: string) => {
    ...
    if (state?.isPositive) {
      navigate('/positive-flow');
    } else if (state?.isSOS) {
      navigate('/calm-mode');
    } else {
      navigate(`/intensity/${stateId}`);
    }
  };
    ...
    <PageTransition>
      <div className="screen-container">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-extrabold text-foreground mb-3">בדיקה מהירה</h2>
          <p className="text-lg text-muted-foreground leading-relaxed">
            לפעמים עוזר לעצור רגע ולבדוק מה קורה בפנים.
            <br />
            <span className="font-semibold text-foreground/80">בחר את מה שהכי קרוב למה שאתה מרגיש עכשיו.</span>
          </p>
        </div>
          ...
          {emotionalStates.map((state, i) => {
            ...
            return (
              <motion.button
                ...
                className={`w-full flex items-center gap-4 p-4 rounded-2xl border text-right transition-all ${colors.bg} ${colors.border}`}
              >
                <Icon size={28} strokeWidth={1.5} className={colors.icon} />
