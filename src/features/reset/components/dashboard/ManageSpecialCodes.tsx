...
export default function ManageSpecialCodes({ specialCodes, refreshSpecialCodes, onBack }: Props) {
  ...
  const toggleActive = async (sc: SpecialCode) => {
    setSaving(sc.id);
    const { error } = await supabase.from('special_codes').update({ active: !sc.active }).eq('id', sc.id);
    if (error) { toast.error('שגיאה בעדכון'); }
    else { toast.success(sc.active ? `קוד ${sc.code} הושבת` : `קוד ${sc.code} הופעל`); await refreshSpecialCodes(); }
    setSaving(null);
  };

  const setExpiry = async (sc: SpecialCode) => {
    const days = parseInt(expiryInputs[sc.id] || '');
    if (!days || days < 1) { toast.error('יש להזין מספר ימים תקין'); return; }
    setSaving(sc.id);
    const expiresAt = new Date(Date.now() + days * 24 * 60 * 60 * 1000).toISOString();
    const { error } = await supabase.from('special_codes').update({ expires_at: expiresAt, active: true }).eq('id', sc.id);
    if (error) { toast.error('שגיאה בעדכון'); }
    else { toast.success(`קוד ${sc.code} יפוג בעוד ${days} ימים`); setExpiryInputs(prev => ({ ...prev, [sc.id]: '' })); await refreshSpecialCodes(); }
    setSaving(null);
  };

  const clearExpiry = async (sc: SpecialCode) => {
    setSaving(sc.id);
    const { error } = await supabase.from('special_codes').update({ expires_at: null }).eq('id', sc.id);
    if (error) { toast.error('שגיאה בעדכון'); }
    else { toast.success(`תפוגה הוסרה מקוד ${sc.code}`); await refreshSpecialCodes(); }
    setSaving(null);
  };
  ...
  return (
    <div className="bg-background p-4 md:p-6 max-w-3xl mx-auto" dir="rtl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-xl bg-[hsl(290,40%,92%)] flex items-center justify-center">
          <Key size={20} className="text-[hsl(290,40%,40%)]" />
        </div>
        <div>
          <h2 className="text-lg font-bold text-foreground">ניהול קודים מיוחדים</h2>
          <p className="text-xs text-muted-foreground">הפעלה, השבתה והגדרת תפוגה</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card/60 p-4 shadow-sm">
        <div className="space-y-3">
          {manageable.map(sc => (
            <div key={sc.id} className={`rounded-xl border border-border/60 bg-card p-4 border-r-4 ${sc.active && !isExpired(sc) ? 'border-r-primary' : 'border-r-destructive'}`}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                />
                <button onClick={() => setExpiry(sc)} disabled={saving === sc.id} className="text-xs py-1.5 px-3 rounded-lg bg-secondary text-secondary-foreground hover:bg-secondary/80">
                  הגדר תפוגה
                </button>
                {sc.expiresAt && (
                  <button onClick={() => clearExpiry(sc)} disabled={saving === sc.id} className="text-xs text-muted-foreground hover:text-foreground">
                    הסר תפוגה
                  </button>
                )}
              </div>
