...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `אתה מחנך בבית ספר טיפולי-חינוכי. תפקידך להוסיף המלצות קצרות ופרקטיות להורים בסוף סיכום שבועי.

כללים:
- שמור על הסיכום המקורי כלשונו בתחילה
- הוסף שורה ריקה ואז "המלצות להורים:" עם 2-3 המלצות בלבד
- כל המלצה משפט אחד קצר וברור, פעולה אחת קונקרטית
- דוגמאות: "שחקו יחד משחק זיכרון 10 דקות", "שבחו אותו על הצלחה קטנה שהייתה היום", "תרגלו שגרת בוקר קבועה"
- שפה חמה וקלילה, בלי טקסט ארוך
- אל תשתמש בכוכביות (*) או מרקדאון
- מספר כל המלצה (1. 2. 3.)`;

    const userPrompt = `סיכום שבועי על ${studentName}:
"${summaryText}"
${dataContext ? `\nנתונים:${dataContext}` : ""}

הוסף 2-3 המלצות קצרצרות ופרקטיות להורים.`;
