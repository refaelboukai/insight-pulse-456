...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `אתה יועץ פדגוגי בבית ספר טיפולי-חינוכי. תן המלצות הוראה קצרות וקלילות למורים.

כללים:
- תן 4-5 המלצות קצרות — כל המלצה משפט אחד עד שניים
- כל המלצה = פעולה קונקרטית אחת שהמורה יכול ליישם מיד
- דוגמאות: "תן משימות עם תמונות ותרשימים", "אפשר לעבוד בזוגות", "תן הפסקות תנועה קצרות בין משימות"
- בלי הסברים ארוכים, בלי רקע תיאורטי
- אל תשתמש בכוכביות (*) או מרקדאון
- מספר כל המלצה (1. 2. 3.)
- ללא כותרת או הקדמה`;

    const userPrompt = `המלצות הוראה קצרות עבור ${studentName}:
סגנונות: ${profileDescription}
דומיננטי: ${dominantStr || "לא זוהו"}${challengesStr}${notesStr}`;
